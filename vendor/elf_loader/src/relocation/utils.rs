    ...
    PostH: RelocationHandler + ?Sized,
{
    ...
    #[inline]
    /// like find_symbol, but an undefined weak symbol resolves to 0,
    /// matching the system loader; old crti-style init code null-checks
    /// exactly these slots
    pub(crate) fn find_symbol_or_weak0(&mut self, r_sym: usize) -> Option<RelocValue<usize>> {
        if let Some(v) = self.find_symbol(r_sym) {
            return Some(v);
        }
        let (dynsym, _) = self.core.symtab().symbol_idx(r_sym);
