...
/// A per-thread marker for recursive-dlopen detection: the address of a
/// thread-local is unique per live thread.
fn thread_marker() -> usize {
    std::thread_local! { static MARK: u8 = const { 0 }; }
    MARK.with(|m| m as *const u8 as usize)
}

fn get_env(name: &str) -> Option<&'static str> {
    // hosts without a discoverable `environ` (static-pie, alternative
    // libcs) still have a perfectly good std
    #[cfg(feature = "std")]
    if unsafe { crate::core_impl::types::ENVP.is_null() } {
...
struct OpenContext<'a> {
    ...
    committed: bool,
    /// Why the most recent existing candidate failed to load: without
    /// this, a broken library reads as a missing one.
...
impl<'a> Drop for OpenContext<'a> {
    fn drop(&mut self) {
        ...
        if !self.committed {
            ...
            // Existing entries this failed session marked RELOCATING were
            // relocated before it borrowed them; restore their state so later
            // opens don't wait on a session that no longer exists.
            let marker = thread_marker();
            for lib in &self.dep_libs {
                let name = lib.shortname();
                if self.added_names.iter().any(|n| n == name) {
                    continue;
                }
                if let Some(entry) = lock.get_mut(name) {
                    if entry.relocating_owner == marker && !entry.state.is_relocated() {
                        entry.state.set_relocated();
                        entry.relocating_owner = 0;
                    }
                }
            }
        }
...
impl<'a> OpenContext<'a> {
    ...
    fn wait_for_library(&mut self, shortname: &str) -> Option<GlobalDylib> {
        loop {
            ...
            match entry {
                ...
                Some(lib)
                    if lib.relocating_owner != 0 && lib.relocating_owner == thread_marker() =>
                {
                    // A recursive dlopen from an initializer of a session on this
                    // thread. Hand the entry back as ld.so does; waiting would be
                    // waiting on ourselves.
                    log::debug!("dlopen: recursive open of [{}] during relocation", shortname);
                    return Some(lib);
                }
                Some(_) => {
    ...
    fn relocate(&mut self, order: &[usize], deps: &Arc<[LoadedDylib]>) -> Result<()> {
        ...
        for &idx in order {
            ...
            lib.relocator()
                .scope(scope.cloned())
                .pre_find_fn(crate::api::dl_builtin)
                .lazy(is_lazy)
    ...
    fn find_library(
            ...
            .chain(runpath.iter())
        {
            ...
            match self.try_load_internal(rpath, runpath, &candidate, bytes) {
                Ok(()) => return Ok(()),
                Err(e) => self.note_candidate(&candidate, &e),
            }
        }
        ...
        Err(find_lib_error(format!(
            "can not find library: {} (last error: {:?})",
            lib_name, self.last_load_error
        )))
    }

    fn note_candidate(&mut self, path: &ElfPath, e: &crate::error::Error) {
        if std::path::Path::new(path.as_str()).exists() {
            self.last_load_error = Some(format!("{}: {e:?}", path.as_str()));
        }
    }
...
pub unsafe extern "C" fn dlopen(filename: *const c_char, flags: c_int) -> *const c_void {
    let lib = if filename.is_null() {
        ...
        let Ok(path) = filename.to_str() else {
            crate::api::dlerror::set_last_error("invalid library path".to_string());
            return core::ptr::null();
        };
        match ElfLibrary::dlopen(path, flags) {
            Ok(lib) => lib,
            Err(e) => {
                crate::api::dlerror::set_last_error(format!("{}: {}", path, e));
                return core::ptr::null();
            }
        }
    };
...
/// # Safety
/// It is the same as `dlmopen`. Namespaces are not isolated: everything
/// shares the base namespace, the only sound choice with a single libc in
/// the process.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn dlmopen(
    lmid: core::ffi::c_long,
    filename: *const c_char,
    flags: c_int,
) -> *const c_void {
    if lmid != 0 {
        log::warn!("dlmopen: namespace {} requested, loading into base", lmid);
    }
